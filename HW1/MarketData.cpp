...
int RetrieveMarketData(const string& url_request, string& read_buffer)
{
    ...
    //cout << "----- buffer data looks like this: -----"<<endl;
...
int PopulateIntradayTrades(const string& read_buffer, Stock& stock)
{
    ...
    std::cout << "\nSucceed in parsing json for intraday data \n" << root << endl;
    ...
    for (Json::Value::const_iterator itr = root.begin(); itr != root.end(); itr++)
    {
        timestamp = (*itr)["timestamp"].asInt();
        // datetime = (*itr)["datetime"].asString();
