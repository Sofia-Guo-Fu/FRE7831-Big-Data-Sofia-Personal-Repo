...
#include <fstream> // include header for file stream operations
...
int main(void)
{
    ...
    string urlRequest = url_common + symbol + ".US?" + "from=" + start_date + "&to=" + end_date + "&api_token=" + api_token + "&interval=5m&fmt=json"; //change the url request to download intraday
    ...
    {
        cout<<"Error: Unable to open the file 'Results.txt' "<<endl;
        return -1;
    }
    ...
    // close the file
    outputFile.close();
    cout<<"Stock data successfully written to 'Results.txt'. "<<endl;
