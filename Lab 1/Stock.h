...
protected:
    ...
    int volume;
public:
    Trade(float open_, float high_, float low_, float close_, int volume_):open(open_), high(high_),low(low_),close(close_),volume(volume_){}
    ~Trade(){}
...
public:
    ...
    {
        out << "Date: "<<t.date<<"Timestamp: "<<t.timestamp<<"Open: "<<t.open<<"High: "<<t.high<<"Low: "<<t.low<<"Close: "<<t.close<<"Volume: "<<t.volume<<endl;
        return out;
    }
};
